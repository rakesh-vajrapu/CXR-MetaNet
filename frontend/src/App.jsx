...
export default function App() {
  ...
  const handleDownloadPDF = async (reportType) => {
    ...
    doc.text(`AI Confidence: ${results ? (results.confidence_score * 100).toFixed(1) + '%' : 'N/A'}`, 110, 67);
    ...
    // Print text lines, simple pagination if it exceeds page limits
    for (let i = 0; i < textLines.length; i++) {
      if (currentY > 280) {
        doc.addPage();
        currentY = 20;
      }
      doc.text(textLines[i], 15, currentY);
      currentY += 7;
    }

    // Use file-saver to handle Chrome async gesture download issue
  ...
  const handleChatSubmit = async (e) => {
    ...
    try {
      ...
      if (results) {
        contextParts.push(`Current Diagnosis: ${results.prediction} (${(results.confidence_score * 100).toFixed(1)}% confidence). Heatmap shows: ${results.heatmap_description || 'N/A'}`);
      }
        ...
        >
          ...
          <div className="z-10">
            ...
            <p className={`text-[9px] sm:text-[10px] md:text-[11px] font-semibold mt-1 tracking-wide transition-colors duration-300 ${theme === 'dark' ? 'text-cyan-400/70' : 'text-indigo-500/80'} font-josefin-sans`}>
              CXR-MetaNet: Chest X-Ray Meta-Ensemble Neural Network using Hybrid CNN–Transformer Architecture
            </p>
                          ...
                          <div className="w-full h-full pr-2 flex flex-col relative">
                            ...
                            <div className="overflow-y-auto custom-scrollbar flex-1 pb-2">
                              {activeReportTab === 'radiologist' ? (
                                <div className={`text-[14px] leading-[1.85] whitespace-pre-wrap p-6 rounded-xl border shadow-inner
                                  ${theme === 'dark' ? 'bg-black/30 border-white/5 text-gray-200' : 'bg-gray-50 border-gray-200 text-gray-700'}`}
                                  style={{ fontFamily: "'DM Sans', sans-serif", fontWeight: 400, letterSpacing: '0.01em' }}>
                                  {reportsData.radiologist}
                                </div>
                              ) : (
                                <div className={`text-[15px] leading-[1.9] whitespace-pre-wrap p-6 rounded-xl border shadow-inner
                                  ${theme === 'dark' ? 'bg-gradient-to-br from-[#00D4FF]/5 to-[#FF0080]/5 border-white/5 text-gray-200' : 'bg-gradient-to-br from-blue-50/50 to-pink-50/50 border-gray-200 text-gray-700'}`}
                      ...
                      onClick={() => {
                        ...
                        const autoInit = async () => {
                          ...
                          if (results && results.prediction) {
                            initPrompt = `We just ran an analysis resulting in a ${results.prediction} diagnosis with ${(results.confidence_score * 100).toFixed(1)}% confidence. Briefly greet me as the AI Assistant, acknowledge this specific diagnosis if applicable, and offer to explain the heatmap or answer questions. Be very concise, maximum 1 sentence.`;
                          } else {
                            initPrompt = "Briefly greet me as the System AI Assistant. Offer to answer any general medical or technical questions about the CDSS. Be very concise, maximum 1 sentence. Do NOT mention that a scan is uploaded or not.";
                          }
